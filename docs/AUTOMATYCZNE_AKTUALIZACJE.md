...
Model `@cf/meta/llama-3.3-70b-instruct-fp8-fast`: limit automatu 8000 neuronów/dobę UTC. Rezerwacja przed wywołaniem zależy od górnej granicy wejścia w bajtach UTF-8 i limitu wyjścia. Licznik pokazuje osobno zużycie wyliczone z tokenów zwróconych przez model, margines 10% + 50 neuronów na wywołanie, oszacowania odpowiedzi bez pomiaru, niepewne próby oraz trwające rezerwacje. Zakończona odpowiedź jest rozliczana także wtedy, gdy jej JSON nie przechodzi walidacji. Niepewny wynik zachowuje rezerwację; historyczny licznik nie jest zerowany podczas migracji. Błąd `ai_local_budget_reserved` oznacza lokalną blokadę ostrożnościową, nie wyczerpanie limitu konta Cloudflare. Wynik modelu jest pamiętany niezależnie od wersji walidatora. Błąd dla niezmienionego źródła ma pierwsze ponowienie po 15 minutach, kolejne po 6 godzinach. Odpowiedź obejmuje maksymalnie 24 najważniejsze fakty ze wszystkich sekcji; wynik jest celowo oznaczony jako częściowy. Osiągnięcie limitu blokuje kolejne wywołania bez zmiany planu usługi. Limit API 120 s, limit odczytu źródła 25 s, ograniczona wielkość odpowiedzi i lista dozwolonych hostów.
...
### Kontrola limitów i transkrypcji 16.09.2026

Wcześniejszy komunikat o wyczerpaniu limitu usługi był nieprecyzyjny. Panel Supadata 15.09 pokazał 5/100 kredytów (trzy strony wyszukiwania, metadane jednego filmu i jego napisy), a Cloudflare około 3,7 tys. z 10 tys. darmowych neuronów. Blokadę wywołały lokalne rezerwacje podczas prób, nie 100 pobranych filmów. Limity i plany pozostają bez zmian.
