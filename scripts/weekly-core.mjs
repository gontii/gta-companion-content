...
export function extractDateRange(html, { publishedWeekId, now = new Date() } = {}) {
  let headlineText = '';
  try {
...
export function extractPublishedWeekId(html) {
  const metaMatch =
...
async function fetchRockstarGraphql(query, variables, fetchImpl = safeFetch) {
...
export function rockstarPostToHtml(post, sourceUrl) {
  const created = parseRockstarCreated(post?.created) || post?.created_formatted || '';
...
export async function resolveRockstarNewswireSource(fetchImpl = safeFetch, { membership = false } = {}) {
  ...
  const posts = (listData?.posts?.results || []).filter(p => /GTA\+|GTA Plus/i.test(p.title || '') === membership);
  const postSummary = findRockstarNewswirePost(posts);
...
export async function resolveRockstarMonthlySource() {
  return resolveRockstarNewswireSource(safeFetch, { membership: true });
}
...
export async function resolveRockstarIntelSource() {
  try {
...
export async function resolveGtabaseSource() {
  try {
...
export async function resolveSourceCandidates(sourceUrl) {
